...
class ZendR_Application_Resource_Doctrine extends Zend_Application_Resource_ResourceAbstract
{
    /**
     * @var array
     */
    ...
    {
        $reflect = new ReflectionClass('ZendR_Doctrine_Core');
    ...
    protected function _getCache(array $options)
    {
        ...
        switch ($options['driver'])
        {
            ...
            case 'memcache':
                ...
                return new ZendR_Cache_Memcache($options['options']);
     ...
     *
     * @param   array $options
     * @return  ZendR_Application_Resource_Doctrine
     */
    ...
    protected function _initManager()
    {
        ...
        $manager->setAttribute(Doctrine_Core::ATTR_MODEL_LOADING, ZendR_Doctrine_Core::MODEL_LOADING_ZEND); // default
    ...
    public function init()
    {
        if (!class_exists('Doctrine_Core')) {
            throw ZendR_Doctrine_Exception::doctrineNotFound();
        }
        spl_autoload_register(array('Doctrine_Core', 'autoload'));

        $manager = $this->_initManager();
        $connections = $this->_initConnections();
        ZendR_Doctrine_Core::loadAllZendModels();
        return new ZendR_Doctrine_Registry($manager, $connections, $this->_paths, $this->_generateModelOptions);
    }
}
